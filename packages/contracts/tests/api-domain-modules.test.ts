...
import { apiActivityImageSchema } from "@yonyoung/contracts/activities";
import { apiAuditLogSchema } from "@yonyoung/contracts/audit";
import {
  ATTACHMENT_ACCEPT,
  apiAttachmentSchema,
  apiCreateAttachmentInputSchema,
} from "@yonyoung/contracts/attachments";
import { API_ERROR_CODES, apiRoleSchema } from "@yonyoung/contracts/common";
import { apiAdminDashboardStatsSchema } from "@yonyoung/contracts/dashboard";
import { apiExhibitionImageSchema } from "@yonyoung/contracts/exhibitions";
import { apiGenerationSchema } from "@yonyoung/contracts/generations";
import { apiLinktreeSchema } from "@yonyoung/contracts/linktree";
import { apiRecruitingPlanSchema } from "@yonyoung/contracts/recruiting";
import {
  DEFAULT_SITE_SETTINGS,
  apiSiteSettingsSchema,
} from "@yonyoung/contracts/site-settings";
import { apiPresignResponseSchema } from "@yonyoung/contracts/uploads";
import { apiUserSchema } from "@yonyoung/contracts/users";
import * as legacyContracts from "@yonyoung/contracts";
import * as legacySchemas from "@yonyoung/contracts/schemas";
...
describe("shared/contracts/api domain modules", () => {
  it("keeps the compatibility facade bound to every domain module", () => {
    ...
    expect(legacySchemas.apiExhibitionImageSchema).toBe(
    ...
    expect(legacySchemas.apiPresignResponseSchema).toBe(
  ...
  it("accepts each valid attachment variant and rejects non-http links", () => {
    expect(
      apiCreateAttachmentInputSchema.safeParse({
        scope: "site_donate",
        title: "외부 문서",
        linkUrl: "https://example.com/document",
      }).success,
    ...
    expect(
      apiCreateAttachmentInputSchema.safeParse({
        scope: "site_donate",
        title: "외부 문서",
        linkUrl: "ftp://example.com/document",
      }).success,
    ).toBe(false);
  });
});
