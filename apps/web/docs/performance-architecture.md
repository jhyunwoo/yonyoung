...
API(`apps/api`)도 공개 응답을 엣지/KV에 최대 120초(+stale 300초) 캐시한다. Next 캐시는
`updateTag` 로 즉시 버려지지만, 그 직후의 재조회가 API 캐시의 수정 전 응답을 받으면
옛 데이터가 `days` 수명으로 다시 구워진다. 그래서 Next 서버의 공개 읽기는 모두
`?fresh=1` 을 붙인다(`server/http/fresh-public-read.ts`). API 는 이 요청을 어떤 캐시에도
읽거나 쓰지 않고 `no-store` 로 돌려준다. 브라우저가 프록시로 직접 부르는 공개 API
(조회수 등)에는 붙이지 않는다 — API 캐시는 그 트래픽을 위한 것이다.
