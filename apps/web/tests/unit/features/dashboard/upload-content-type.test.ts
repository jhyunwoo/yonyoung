...
describe("resolveUploadContentType", () => {
  it("Windows가 보고하는 zip 별칭을 표준 MIME으로 바꾼다", () => {
    expect(
      resolveUploadContentType(file("자료.zip", "application/x-zip-compressed")),
    ).toBe("application/zip");
  });

  it("브라우저가 타입을 비우면 확장자로 판정한다", () => {
    expect(resolveUploadContentType(file("회계.hwp", ""))).toBe("application/x-hwp");
    expect(resolveUploadContentType(file("회계.HWPX", ""))).toBe(
  ...
  it("허용된 보고값은 그대로 쓴다", () => {
    ...
    expect(
      resolveUploadContentType(file("a.pdf", "application/pdf; charset=binary")),
    ).toBe("application/pdf");
  });

  it("판정할 수 없는 파일은 이미지로 위장하지 않고 원래 값을 서버에 넘긴다", () => {
    expect(resolveUploadContentType(file("run.exe", ""))).toBe(
