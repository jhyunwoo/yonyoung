...
- **이미지 로딩 스켈레톤**: 프레임에 `data-image-skeleton` 을 붙이면 `globals.css` 의 `[data-image-skeleton]::before` 가 스윕 스켈레톤을 그리고, 아일랜드 `ImageSkeletonObserver` 하나가 document 전체에서 load/error 를 캡처로 받아 끝난 프레임에 `data-image-loaded` 를 붙여 끈다(이미지 장수와 무관하게 하이드레이션되는 컴포넌트는 이것 1개다 — 이미지마다 상태 래퍼를 두면 갤러리에서 장수만큼 늘어난다). 함정 4가지: ① `::before` 는 DOM 순서상 이미지 **아래**에 깔린다 — 덕분에 JS 가 죽어도 사진이 보이고 캐시된 이미지는 깜빡임이 없다. 이미지를 `opacity:0` 으로 숨겼다 켜는 방식으로 바꾸면 두 성질을 다 잃는다 ② 그래서 **프레임을 꽉 채우는 불투명 이미지**에만 쓴다(투명 PNG·`object-contain` 은 뒤가 비친다 — 푸터 로고가 그래서 제외다) ③ 아일랜드가 스트리밍 콘텐츠보다 먼저 속성을 붙일 수 있어 프레임마다 `suppressHydrationWarning` 이 필요하다 ④ 조건부 이미지 자리에는 속성도 조건부로 붙인다(이미지가 없는 프레임은 아일랜드가 즉시 걷지만, 표시 자체를 안 붙이는 게 맞다). 계약 테스트 `tests/unit/app-shared/public-image-skeleton-contract.test.ts` 가 누락을 잡는다
...
- **공개 라우트(`app/(home)`)에서 framer-motion 을 import 하지 않는다.** 헤더가 모든 공개 페이지에 있어서 import 하나가 공개 번들에 gzip 약 43KB 를 얹는다. 등장·패럴랙스·호버·오버레이 전환은 `globals.css` 와 아일랜드 세 개(`RevealObserver`, `HeroParallax`, `ImageSkeletonObserver`) + `shared/react/use-mount-transition.ts` 로 구현돼 있다. `tests/unit/app-shared/public-bundle-policy.test.ts` 가 이를 고정한다. 대시보드는 계속 써도 된다
