...
const resolveActivitiesData = (
  ...
  const visibleActivities =
    generationScoped && scopedGeneration
      ? activities.filter(
          (activity) => activity.generationId === scopedGeneration.id,
        )
      : generationScoped
        ? []
        : activities;
...
export default function ActivitiesAdminPage({
  ...
  const applyLoadedData = (
    ...
    const { scopedGeneration, visibleGenerations, visibleActivities } =
      resolveActivitiesData(
        activities,
        generationList,
        generationScoped,
    ...
    const nextImageId =
      selectedImageId &&
      selectedActivity?.detailImages.some(
    ...
    <div className="space-y-6" data-testid="activities-page">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {generationScoped ? (
              <p
                ...
                data-testid="activities-scoped-generation"
              >
                {scopedGeneration
      ...
      {!isStandaloneRoute ? (
        <AdminInfoBox title="작업 안내">
          커버/세부 이미지는 파일 선택 즉시 업로드됩니다. 세부 이미지는 여러
          장을 드래그 앤 드롭으로 추가하고, 업로드 후 목록에서 드래그해 순서를
          변경할 수 있습니다.
        </AdminInfoBox>
      ) : null}
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-500">
                        활동 기간: {formatTimestamp(item.startDate)} ~{" "}
        ...
        {panelMode === "create" ? (
          ...
          <div className="space-y-4">
            ...
            <section className="rounded-lg border border-gray-200 bg-gray-50 p-4">
              ...
              <p className="mt-1 text-sm text-gray-600">
                활동 기간: {formatTimestamp(selected.startDate)} ~{" "}
            ...
            {sortedSelectedDetailImages.length > 0 ? (
              <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
                {sortedSelectedDetailImages.map((image) => (
                  <li
                    key={image.id}
                    className="relative aspect-[4/3] overflow-hidden rounded-md border border-gray-200 bg-gray-50"
                  >
                    <span className="absolute left-2 bottom-2 z-10 rounded-md bg-black/60 px-2 py-1 text-xs font-medium text-white">
                      정렬{" "}
            ...
            >
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    파일 선택 창은 하나만 사용합니다. 선택한 이미지는 병렬로
                    업로드되며, 목록에서 미리보기와 순서를 함께 관리할 수
                    있습니다.
                  </p>
                  <p className="mt-1 text-xs text-gray-500">
                ...
                {detailBatchUpload.items.length > 0 ? (
                  <div
                    className="space-y-2"
                    data-testid="activity-detail-upload-queue"
                  >
                      ...
                      {detailBatchUpload.items.map((item) => (
                        <li
                          ...
                          onDrop={(event) => {
                            ...
                            if (
                              !editQueuedDragId ||
                              editQueuedDragId === item.id
                            ) {
                              return;
                            }
                            detailBatchUpload.moveItem(
                              editQueuedDragId,
                              item.id,
                            );
              ...
              <div className="mt-2 flex gap-2">
                ...
                <AdminActionButton
                  ...
                  disabled={
                    isSubmitting && activeSubmitAction !== "deleteActivity"
                  }
                  loadingText="활동 삭제 중..."
