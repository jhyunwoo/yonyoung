  ...
  return (
    <div className="pb-16 md:pb-20">
      <section className="px-4 pb-10 pt-14 md:px-6 md:pb-14 md:pt-18">
        <div className="mx-auto w-full max-w-6xl rounded-3xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] p-6 shadow-[0_20px_70px_-42px_var(--shadow-strong)] md:p-10">
          <MotionReveal>
      ...
      >
        <div
          className="relative border-l border-[var(--surface-border)] pl-5 md:pl-8"
          data-testid="about-history"
        >
          <MotionReveal className="mb-6">
            <article className="relative rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] p-5">
              <span className="absolute -left-[2.15rem] top-6 h-3 w-3 rounded-full bg-[var(--accent)] md:-left-[2.7rem]" />
              <p className="text-sm font-semibold text-[var(--text-primary)]">1966</p>
          ...
          {generations.map((generation, index) => (
            <MotionReveal key={generation.id} delay={index * 0.04} className="mb-4">
              <article
                className="relative rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] p-5"
                data-testid={`about-generation-card-${generation.id}`}
              >
                <span className="absolute -left-[2.15rem] top-6 h-3 w-3 rounded-full bg-[var(--surface-border)] md:-left-[2.7rem]" />
          ...
          {generations.length === 0 ? (
            <MotionReveal>
              <article className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] p-5 text-sm text-[var(--text-secondary)]">
                공개된 기수 정보가 없습니다.
              </article>
            </MotionReveal>
          ) : null}
