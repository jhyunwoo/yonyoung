...
describe("admin-generation utilities", () => {
  const generations = [
  ...
  it("president는 모든 기수를 정렬된 순서로 접근한다", () => {
    ...
    expect(result.map((item) => item.id)).toEqual(["g1", "g1b", "g2"]);
  });

  it("일반 사용자는 본인 generationId와 일치하는 기수만 접근한다", () => {
    const result = getAccessibleGenerations(
      {
        user: {
          role: "regular_member",
  ...
  it("세션이 없거나 generationId가 없으면 빈 배열을 반환한다", () => {
  ...
  it("buildGenerationPath는 resource path 유무에 따라 경로를 생성한다", () => {
  ...
  it("extractGenerationRouteContext는 admin 라우트 컨텍스트를 파싱한다", () => {
  ...
  it("extractGenerationRouteContext는 유효하지 않은 경로에서 null 컨텍스트를 반환한다", () => {
