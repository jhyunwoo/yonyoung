...
describe("RBAC routes",() => {
  ...
  it("운영진은 본인 소속 기수 사용자 목록만 조회할 수 있다",async () => {
  ...
  it("운영진은 본인 기수 외 사용자 상세 조회가 불가하다",async () => {
    const app = createTestApp({
      actor: createActor("manager", IDs.manager, IDs.generation),
      dataService: createDataServiceMock({
  ...
  it("운영진은 본인 기수 사용자 상세 조회가 가능하다",async () => {
    const app = createTestApp({
      actor: createActor("manager", IDs.manager, IDs.generation),
      dataService: createDataServiceMock({
        getUserById: vi.fn(async () =>
  ...
  it("부원은 본인 프로필(image)만 수정 가능하다",async () => {
  ...
  it("미인증 사용자는 본인 기본 정보를 수정할 수 있다",async () => {
    const updateUser = vi.fn(async () => createUser(IDs.member, "unverified"));
    const app = createTestApp({
