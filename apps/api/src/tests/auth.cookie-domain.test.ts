...
describe("resolveCrossSubDomainCookieDomain", () => {
  ...
  it("서브도메인이 없으면 undefined를 반환한다", () => {
    expect(resolveCrossSubDomainCookieDomain("https://moveto.kr")).toBeUndefined();
  });

  it("localhost/IP 호스트는 undefined를 반환한다", () => {
    expect(resolveCrossSubDomainCookieDomain("http://localhost:8787")).toBeUndefined();
    expect(resolveCrossSubDomainCookieDomain("http://127.0.0.1:8787")).toBeUndefined();
  });
